...
def update_package_status(packageID):
    ...
    owner = find_owner(packageID)
    if(db.sismember(f"packages:{user}",packageID)) or (pattern.fullmatch(user)):
        new_status = update_package(packageID)
        return jsonify(msg = f'Package status updated (owner:{owner})', newStatus = new_status ,packageID = packageID), 200
    else:
...
def update_package(packageID):
    if(db.hexists(f"package:{packageID}","id")):
        ...
        if(status == "Generated"):
            ...
            db.rpush(f'notifications:{owner}', f'Package {packageID} status updated: In Transit')
            return "In transit"
        elif(status == "In transit"):
            db.hset(f"package:{packageID}","status","Delivered")
            db.rpush(f'notifications:{owner}', f'Package {packageID} status updated: Delivered')
            return "Delivered"
        elif(status == "Delivered"):
            db.hset(f"package:{packageID}","status","Recived")
            db.rpush(f'notifications:{owner}', f'Package {packageID} status updated: Recived')
            return "Recived"
        return status
    else:
        return False
...
def find_owner(packageID):
    for i in db.keys("packages:*"):
        if db.sismember(i,packageID):
            tmp = str(i).split(':')[1]
            return tmp[:-1]
